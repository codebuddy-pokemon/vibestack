...
function RegisterForm() {
    ...
    const handleSocialLogin = (provider: string) => {
        signIn(provider, { callbackUrl: "/projects/new" })
    }

    return (
        <Card className="w-full max-w-md relative z-10 border-white/10 bg-black/50 backdrop-blur-xl">
            <CardHeader>
                <CardTitle className="text-2xl font-bold bg-gradient-to-r from-white to-white/60 bg-clip-text text-transparent">Create an Account</CardTitle>
                <CardDescription>
                    {plan === "PRO" ? (
                        <span className="text-purple-400 font-medium">You selected the Pro Plan. Great choice!</span>
                    ) : (
                        "Join VibeStack to start generating amazing landing pages."
                    )}
                </CardDescription>
            </CardHeader>
            <CardContent>
                <div className="grid grid-cols-2 gap-4 mb-6">
                    <Button variant="outline" onClick={() => handleSocialLogin('github')} className="bg-white/5 border-white/10 hover:bg-white/10 hover:text-white">
                        <Github className="mr-2 h-4 w-4" /> Github
                    </Button>
                    <Button variant="outline" onClick={() => handleSocialLogin('google')} className="bg-white/5 border-white/10 hover:bg-white/10 hover:text-white">
